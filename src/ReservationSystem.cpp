...
bool ReservationSystem::passengerExists(int id) const {
...
bool ReservationSystem::flightExists(const std::string& flightNumber) const {
    for (const auto& flight : flights) {
        if (flight.flightNumber == flightNumber) {
            return true;
        }
...
bool ReservationSystem::bookFlight(int passengerId, const std::string& flightNumber) {
    ...
    // Check if passenger already has a booking on this flight
    for (const auto& booking : bookings) {
        if (booking.passenger.id == passengerId && 
            booking.flight.flightNumber == flightNumber && 
            booking.status == "Confirmed") {
            std::cout << "❌ Error: Already booked on this flight!\n";
            return false;
        }
    }
    ...
    // Check availability
    if (!flight->bookSeat()) {
        std::cout << "❌ Error: No seats available!\n";
        return false;
    }
    ...
    for (auto& booking : bookings) {
        if (booking.bookingId == bookingId) {
            if (booking.status == "Cancelled") {
                std::cout << "❌ Error: Booking already cancelled!\n";
                return false;
            }
...
// ✅ ADD THIS FUNCTION - Display all bookings
...
// ✅ ADD THIS FUNCTION - Display bookings for a specific passenger
